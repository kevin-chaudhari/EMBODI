...
class Player {
  ...
  onInteraction() {
    ...
    if (!audio.listener) {
      ...
      audio.source = new PositionalAudio(audio.listener);
      audio.source.setMediaElementSource(video);
      audio.source.setRefDistance(20); // Sound drops off after 20 units
      audio.source.setRolloffFactor(1); // Standard rolloff
